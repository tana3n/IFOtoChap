    ...
    while (i < PGCNs){
        ...
        */
        ...
        while (j < PGCN_Cells) {
            ...
            info.playback_ss = uint32_t(double(fN) * 1001.0 / 30000.0) % 60;
            info.playback_hh = uint32_t(double(fN) * 1001.0 / 30000.0) / 60 / 60;
            info.playback_mm = uint32_t(double(fN) * 1001.0 / 30000.0) / 60 % 60;
            info.playback_ms = double(fN) * 1001.0 / 30000.0 - uint32_t(double(fN) * 1001.0 / 30000.0);
            info.playback_no = j + 2;
            writeChapter(fN, option, &info);
